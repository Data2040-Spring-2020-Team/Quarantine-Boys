...
def main():
    ...
    cur_ = 0
    pos_keep = npr.choice(len(pos), size=7500, replace=False)
    sum_ = len(pos_keep)
    print('Writing')
    for i in pos_keep:
        view_bar(cur_, sum_)
    ...
    cur_ = 0
    neg_keep = npr.choice(len(neg), size=23000, replace=False)
    sum_ = len(neg_keep)
    for i in neg_keep:
        line = neg[i]
        view_bar(cur_, sum_)
    ...
    print('\n'+'part random cropped')
    cur_ = 0
    part_keep = npr.choice(len(part), size=23000, replace=False)
    sum_ = len(part_keep)
    for i in part_keep:
        view_bar(cur_, sum_)
        line = part[i]
